...
class RepoDetails(object):
    ...
    def determine_imagename(self, repo_path):
        ...
        with open(az_pipeline_file, 'r') as f:
            return self.grep_imagename(f.readlines(), self.repo["name"])

    @staticmethod
    def grep_imagename(lines, default):
        for line in lines:
            match = re.search(r"[iI]mageName.*:\s*([\"'])?([^\"']*)([\"'])?", line)
            if match is not None:
                return match.group(2).rstrip()

        print("No matching line azure-pipelines.yml")
        return default
