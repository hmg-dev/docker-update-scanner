...
class RepoDetailsTest(unittest.TestCase):
    ...
    def test_determine_imagename_for_missing_azurepipeline(self):
        result = self.sut.determine_imagename(self.dummy_path)
        self.assertEqual(self.dummy_repo["name"], result)

    def test_determine_imagename_for_invalid_name_attribute(self):
        with TemporaryDirectory("dummy-repo") as td:
            test_file = os.path.dirname(__file__) + "/../resources/azure-pipelines.no-imagename.yaml"
            self.assertTrue(Path(test_file).is_file())
            copyfile(test_file, td + "/azure-pipelines.yml")
            result = self.sut.determine_imagename(td)
            self.assertEqual(self.dummy_repo["name"], result)

    def test_determine_imagename_for_valid_pipeline_file(self):
        expected_name = "prj-infra-dummy"
        with TemporaryDirectory("dummy-repo") as td:
    ...
    def test_determine_imagename_for_valid_pipeline_file_without_quotes(self):
        expected_name = "prj-infra-dummy"
        with TemporaryDirectory("dummy-repo") as td:
            test_file = os.path.dirname(__file__) + "/../resources/azure-pipelines.no-quotes.yml"
            self.assertTrue(Path(test_file).is_file())
            copyfile(test_file, td + "/azure-pipelines.yml")
            result = self.sut.determine_imagename(td)
            self.assertEqual(expected_name, result)
